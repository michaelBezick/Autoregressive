...
for std_dev in std_devs:
    print("\n" + "=" * 60)
    print(f"Running experiment for std_dev = {std_dev}")
    print("=" * 60)
    ...
    # copy the list so we don't mutate the same object across runs
    skill_params = list(initial_skill_params)
    ...
    Phi_matrices_estimate = torch.randn(
        (AR_order_p, num_players - 1, num_players - 1)
    )

    skill_params_est = skill_params_est.cuda()
    W_cuda = W.cuda()
    Z_cuda = Z.cuda()
    actual_skill_params_cuda = actual_skill_params.cuda()
    Phi_matrices_estimate = Phi_matrices_estimate.cuda()
    Phi_matrices_cuda = Phi_matrices.cuda()
    ...
    for epoch in tqdm(range(epochs), desc=f"std={std_dev}"):
        ...
        if epoch % STEP == 0:
            ...
            actual_skill_params_normalized = normalize_alpha(
                torch.squeeze(actual_skill_params_cuda)
            )
    ...
    plot_series(
        ...
        epochs_logged,
        true_alphas_error,
        label="Alpha Estimates Error (MSE)",
    )
    ...
    plot_series(
        ...
        epochs_logged,
        true_p_matrix_error,
        label="P Matrix Error (MSE)",
    )
    axes[1].set_title("P Matrix Error (MSE)")
    axes[1].set_xlabel("Epoch")
    axes[1].grid(True)

    fig.suptitle(f"Model Estimation Errors Over Time (std={std_dev})")
    fig.tight_layout()
    fig.savefig(f"errors_std{std_dev}.pdf")
    plt.close(fig)
    ...
    # keep 4th subplot empty for now, or you can add another metric if you want
    axes[3].axis("off")

    fig.suptitle(f"AR Error, BTL Likelihood, and Total Likelihood (std={std_dev})")
    fig.tight_layout()
    fig.savefig(f"likelihoods_std{std_dev}.pdf")
    plt.close(fig)
