...
# ------------------------------------------------------------
# Custom true latent model: nontrivial AR(1) ring-mixing model
# ------------------------------------------------------------
# Why this model?
# - It is not the identity map.
# - It is deterministic, so transition-state MSE has a clean interpretation.
# - It preserves the sum-to-zero identifiability subspace because each column sums to 1.
# - It is interpretable: each player's next skill is mostly their own previous skill,
#   plus a smaller contribution from one neighboring player's previous skill.
#
# Specifically,
#   alpha_t = Phi alpha_{t-1},
#   Phi = (1 - gamma) I + gamma C,
# where C is the cyclic right-shift permutation matrix.
# ------------------------------------------------------------
...
AR_order_p = 1
erdos_renyi_p = 1.0
std_dev = 0.0  # keep deterministic so the transition metric should ideally go to 0
num_timesteps = 30
epochs = 2_000
N_grad_descent = 10
weight = 30.0
...
# ring-mixing strength; 0.0 would be identity, 1.0 would be a pure cyclic permutation
gamma = 1.
...
def ring_shift_matrix(num_players: int, device=None, dtype=torch.float32) -> torch.Tensor:
    """
    Cyclic right-shift permutation matrix C such that (Cx)_i = x_{i-1 mod n}.
    Each column sums to 1.
    """
    if device is None:
        device = torch.device("cpu")
    C = torch.zeros((num_players, num_players), device=device, dtype=dtype)
    for j in range(num_players):
        C[(j + 1) % num_players, j] = 1.0
    return C



def ring_mixing_phi_matrix(
    num_players: int,
    gamma: float,
    device=None,
    dtype=torch.float32,
    ...
    - nontrivial but still easy to interpret
    """
    if not (0.0 <= gamma <= 1.0):
        raise ValueError(f"gamma must be in [0, 1], got {gamma}")
    if device is None:
        device = torch.device("cpu")
    I = torch.eye(num_players, device=device, dtype=dtype)
    C = ring_shift_matrix(num_players, device=device, dtype=dtype)
    Phi = (1.0 - gamma) * I + gamma * C
    return Phi.unsqueeze(0)  # shape (p=1, n, n)
...
def predict_next_skills_from_histories(
...
def transition_diagnostics(
    ...
    if phi_stack_true is not None:
        ...
        out["phi_matrix_mse"] = F.mse_loss(phi_stack_est, phi_stack_true).item()
...
initial_skill_params, _ = setup(num_players, AR_order_p, device=device, identity_ar1=False)
Phi_matrices = ring_mixing_phi_matrix(num_players, gamma=gamma, device=device)
...
# feasible random initialization for the estimated Phi
...
for epoch in tqdm(range(epochs)):
    ...
    if epoch % STEP == 0:
        with torch.no_grad():
            ...
            print("phi matrix MSE vs true Phi:", dyn_stats["phi_matrix_mse"])
            ...
            phi_matrix_mse_history.append(dyn_stats["phi_matrix_mse"])
...
# --- Figure 1: estimation and transition diagnostics ---
...
plot_series(axes[1], epochs_logged, transition_state_mse_history, label="Transition state MSE")
axes[1].set_title("One-Step Transition State MSE")
axes[1].set_xlabel("Epoch")
axes[1].grid(True)

plot_series(axes[2], epochs_logged, phi_matrix_mse_history, label="Phi Matrix Error (MSE)")
axes[2].set_title("Phi Matrix Error (MSE)")
axes[2].set_xlabel("Epoch")
axes[2].grid(True)

fig.suptitle("Ring-Mixing AR(1) Estimation Diagnostics Over Time")
fig.tight_layout()
fig.savefig("ring_mixing_errors.pdf")
...
fig.tight_layout()
fig.savefig("ring_mixing_likelihoods.pdf")

# --- Figure 3: transition state MSE only ---
...
ax.grid(True)
fig.suptitle("Ring-Mixing AR(1) Transition Dynamics")
fig.tight_layout()
fig.savefig("ring_mixing_transition_state_mse.pdf")
...
print("\n========================================")
print("Final ring-mixing AR(1) transition-dynamics diagnostics")
print(f"true model: Phi = (1-gamma) I + gamma C with gamma = {gamma}")
print("transition state MSE on true latent histories:", final_dyn_stats["transition_state_mse"])
print("transition BTL-probability MSE on true latent histories:", final_dyn_stats["transition_btl_mse"])
print("transition-map MSE vs true Phi on true latent histories:", final_dyn_stats["transition_map_mse"])
print("phi matrix MSE vs true Phi:", final_dyn_stats["phi_matrix_mse"])
print("true latent skill drift across adjacent times:", true_skill_drift)
print("estimated latent skill drift across adjacent times:", est_skill_drift)
print("final column-sum constraint residual:", column_sum_residual(Phi_matrices_estimate).item())
print("========================================")
